 ...
 */
export const BUSINESS_VALUE_SCORES = {
    critical: 4,
    high: 3,
    medium: 2,
    low: 1,
 ...
 */
export const MOSCOW_CATEGORY_WEIGHTS = {
    'Must have': 4,
    'Should have': 3,
    'Could have': 2,
    "Won't have": 1,
...
export class MoSCoWCalculator {
    ...
    validate(input) {
        ...
        if (input.businessValue === undefined) {
            ...
            errors.push(`businessValue must be one of: ${validBusinessValues.join(', ')}`);
        }
        ...
        if (input.riskIfNotDelivered === undefined) {
            ...
            errors.push(`riskIfNotDelivered must be one of: ${validRiskLevels.join(', ')}`);
        }
        ...
        if (input.legalRequirement === true && input.businessValue === 'low') {
            warnings.push('Legal requirements typically have higher business value - review classification');
        }
        // Warning: Critical risk with low business value is unusual
        if (input.riskIfNotDelivered === 'critical' &&
            input.businessValue === 'low') {
            warnings.push('Critical risk with low business value is contradictory - review classification');
        }
        return {
    ...
    normalize(result, allResults) {
        ...
        const rank = sortedResults.findIndex((r) => r.category === result.category &&
            r.priority === result.priority &&
            r.businessValue === result.businessValue) + 1;
     ...
     * @private
     */
    calculateInitialPriority(input, _category) {
 ...
 * ```
 */
export function createMoSCoWCalculator(config = {}) {

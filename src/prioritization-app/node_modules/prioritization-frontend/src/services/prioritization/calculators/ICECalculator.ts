 ...
 * ```
 */
export class ICECalculator implements IFrameworkCalculator<ICEInput, ICEResult> {
  private readonly minScore: number;
  private readonly maxScore: number;
  private readonly decimalPlaces: number;
  private readonly confidenceAsPercentage: boolean;
  ...
  getAutoFillSuggestions(item: PrioritizationItem): Partial<ICEInput> & {
    ...
    const suggestions: Partial<ICEInput> & {
      impactConfidence?: number;
      confidenceConfidence?: number;
      easeConfidence?: number;
    } = {};
   ...
   * @private
   */
  private getEaseLevel(ease: number): string {
    if (ease >= 8) return 'very easy';
    if (ease >= 6) return 'easy';
    if (ease >= 4) return 'moderate';
 ...
 */
export function createICECalculator(config: Partial<typeof DEFAULT_CONFIG> = {}): ICECalculator {

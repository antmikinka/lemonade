 ...
 */
export const EISENHOWER_QUADRANT_WEIGHTS: Record<EisenhowerQuadrant, number> = {
  DoFirst: 4,
  Schedule: 3,
  Delegate: 2,
  Eliminate: 1,
 ...
 * ```
 */
export class EisenhowerCalculator implements IFrameworkCalculator<EisenhowerInput, EisenhowerResult> {
  ...
  normalize(result: EisenhowerResult, allResults: EisenhowerResult[]): NormalizedResult {
    ...
    const getCompositeScore = (r: EisenhowerResult): number => {
      const quadrantWeight = EISENHOWER_QUADRANT_WEIGHTS[r.quadrant];

      // Add priority score if available (for finer granularity)
      const priorityBonus = (r.details.priorityScore as number) ? (r.details.priorityScore as number) / 100 : 0;
    ...
    const rank = sortedResults.findIndex(
      (r) => r.quadrant === result.quadrant && r.details.priorityScore === result.details.priorityScore
    ) + 1;
  ...
  getAutoFillSuggestions(item: PrioritizationItem): Partial<EisenhowerInput> & {
    ...
    const suggestions: Partial<EisenhowerInput> & {
      urgentConfidence?: number;
      importantConfidence?: number;
      urgencyLevelConfidence?: number;
      importanceLevelConfidence?: number;
   ...
   * @private
   */
  private determineQuadrant(urgent: boolean, important: boolean): EisenhowerQuadrant {
    if (urgent && important) {
      return 'DoFirst';
   ...
   * @private
   */
  private calculatePriorityScore(input: EisenhowerInput): number {
   ...
   * @private
   */
  private getActionVerb(quadrant: EisenhowerQuadrant): string {
    switch (quadrant) {
      case 'DoFirst':
 ...
 */
export function createEisenhowerCalculator(

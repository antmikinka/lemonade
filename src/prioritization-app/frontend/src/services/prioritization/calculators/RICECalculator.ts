 ...
 * ```
 */
export class RICECalculator implements IFrameworkCalculator<RICEInput, RICEResult> {
  ...
  private readonly minEffort: number;
  private readonly maxEffort: number;
  ...
  calculate(input: RICEInput): RICEResult {
    ...
    const rawScore =
      (input.reach * input.impact * normalizedConfidence) / input.effort;
  ...
  validate(input: Partial<RICEInput>): ValidationResult {
    ...
    if (input.reach !== undefined) {
      if (typeof input.reach !== 'number' || isNaN(input.reach)) {
        ...
        warnings.push(
          'Reach of 0 means no users affected - consider if this item should be prioritized'
        );
      }
    }
    ...
    if (input.impact !== undefined) {
      if (typeof input.impact !== 'number' || isNaN(input.impact)) {
        ...
        errors.push(
          `Impact must be at least ${this.minImpact} (minimal impact level)`
        );
      } else if (input.impact > this.maxImpact) {
        errors.push(
          `Impact must be at most ${this.maxImpact} (massive impact level)`
        );
      } else {
        // Check if impact matches standard scale values
        const standardValues = Object.values(IMPACT_LEVELS);
        if (!standardValues.includes(input.impact as never)) {
          warnings.push(
            `Impact ${input.impact} doesn't match standard scale. Recommended values: ${standardValues.join(', ')}`
          );
        }
      }
    }
    ...
    if (input.confidence !== undefined) {
      if (typeof input.confidence !== 'number' || isNaN(input.confidence)) {
        ...
        if (input.confidence < this.minConfidence) {
          ...
          warnings.push(
            'Zero confidence means you have no certainty in your estimates'
          );
        } else if (input.confidence < 50 && input.confidence > 1) {
          warnings.push(
            `Low confidence (${input.confidence}%) - consider gathering more data before prioritizing`
          );
        }
      }
    }
    ...
    if (input.effort !== undefined) {
      if (typeof input.effort !== 'number' || isNaN(input.effort)) {
        ...
        warnings.push(
          'Very low effort value - ensure this is measured in the correct units (person-months)'
        );
      } else if (input.effort > 12) {
        warnings.push(
          'High effort value (>12 person-months) - consider breaking this into smaller initiatives'
        );
      }
    }
  ...
  normalize(
    ...
    if (maxScore === minScore) {
      ...
      normalizedScore = parseFloat(
        (((result.score - minScore) / (maxScore - minScore)) * 100).toFixed(2)
      );
    }
  ...
  getAutoFillSuggestions(item: PrioritizationItem): Partial<RICEInput> & {
    ...
    const suggestions: Partial<RICEInput> & {
      reachConfidence?: number;
      impactConfidence?: number;
      confidenceConfidence?: number;
      effortConfidence?: number;
  ...
  private getImpactLevel(impact: number): string {
    ...
    const closest = entries.reduce((prev, curr) =>
      Math.abs(curr[1] - impact) < Math.abs(prev[1] - impact) ? curr : prev
    );
    return closest[0];
  }
}
 ...
 * ```
 */
export function createRICECalculator(

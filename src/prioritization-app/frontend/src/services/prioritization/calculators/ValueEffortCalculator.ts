 ...
 */
export const QUADRANT_WEIGHTS: Record<ValueEffortQuadrant, number> = {
  QuickWin: 4,
  MajorProject: 3,
  FillIn: 2,
  Avoid: 1,
 ...
 * ```
 */
export class ValueEffortCalculator implements IFrameworkCalculator<ValueEffortInput, ValueEffortResult> {
  private readonly highValueThreshold: number;
  private readonly highEffortThreshold: number;
  ...
  normalize(result: ValueEffortResult, allResults: ValueEffortResult[]): NormalizedResult {
    ...
    const getCompositeScore = (r: ValueEffortResult): number => {
    ...
    const rank = sortedResults.findIndex(
      (r) => r.quadrant === result.quadrant && r.roi === result.roi && r.value === result.value
    ) + 1;
  ...
  getAutoFillSuggestions(item: PrioritizationItem): Partial<ValueEffortInput> & {
    ...
    const suggestions: Partial<ValueEffortInput> & {
      valueConfidence?: number;
      effortConfidence?: number;
    } = {};
   ...
   * @private
   */
  private determineQuadrant(value: number, effort: number): ValueEffortQuadrant {
 ...
 * ```
 */
export function createValueEffortCalculator(
  config: Partial<typeof DEFAULT_CONFIG> = {}
): ValueEffortCalculator {

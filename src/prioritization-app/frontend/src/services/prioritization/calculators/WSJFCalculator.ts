 ...
 */
export const FIBONACCI_SCALE = [1, 2, 3, 5, 8, 13, 20, 40, 100] as const;
 ...
 * ```
 */
export class WSJFCalculator implements IFrameworkCalculator<WSJFInput, WSJFResult> {
  private readonly minScore: number;
  private readonly maxScore: number;
  private readonly decimalPlaces: number;
  private readonly minJobSize: number;
  private readonly useFibonacciScale: boolean;
  ...
  validate(input: Partial<WSJFInput>): ValidationResult {
    ...
    if (
      input.userBusinessValue !== undefined &&
  ...
  normalize(result: WSJFResult, allResults: WSJFResult[]): NormalizedResult {
    ...
    if (maxScore === minScore) {
      ...
      normalizedScore = parseFloat(
        (((result.score - minScore) / (maxScore - minScore)) * 100).toFixed(this.decimalPlaces)
      );
    }
  ...
  getAutoFillSuggestions(item: PrioritizationItem): Partial<WSJFInput> & {
    ...
    const suggestions: Partial<WSJFInput> & {
      userBusinessValueConfidence?: number;
      timeCriticalityConfidence?: number;
      riskReductionOpportunityConfidence?: number;
      jobSizeConfidence?: number;
   ...
   * @private
   */
  private getWSJFInterpretation(score: number): string {
    if (score >= 10) {
   ...
   * @private
   */
  private isValidFibonacci(value: number): boolean {
    return FIBONACCI_SCALE.includes(value as never);
  }
}
 ...
 * ```
 */
export function createWSJFCalculator(config: Partial<typeof DEFAULT_CONFIG> = {}): WSJFCalculator {

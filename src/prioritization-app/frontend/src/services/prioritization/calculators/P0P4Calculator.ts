 ...
 */
export const PRIORITY_WEIGHTS: Record<P0P4Level, number> = {
  P0: 5,
  P1: 4,
  P2: 3,
  P3: 2,
 ...
 */
export const PRIORITY_TIMEFRAMES: Record<P0P4Level, string> = {
 ...
 * ```
 */
export class P0P4Calculator implements IFrameworkCalculator<P0P4Input, P0P4Result> {
  private readonly p0Threshold: number;
  private readonly p1Threshold: number;
  private readonly p2Threshold: number;
  private readonly p3Threshold: number;
  ...
  normalize(result: P0P4Result, allResults: P0P4Result[]): NormalizedResult {
    ...
    if (maxScore === minScore) {
      ...
      normalizedScore = parseFloat(
        (((result.severityScore - minScore) / (maxScore - minScore)) * 100).toFixed(this.decimalPlaces)
      );
    }
  ...
  getAutoFillSuggestions(item: PrioritizationItem): Partial<P0P4Input> & {
    ...
    const suggestions: Partial<P0P4Input> & {
      baseSeverityConfidence?: number;
      factorsConfidence?: number;
    } = {};
   ...
   * @private
   */
  private calculateSeverityScore(input: P0P4Input): number {
   ...
   * @private
   */
  private calculateFactorsScore(factors: SeverityFactors): number {
    let score = 0;
   ...
   * @private
   */
  private calculateTimeModifier(input: P0P4Input): number {
    let modifier = 0;
   ...
   * @private
   */
  private determinePriority(severityScore: number): P0P4Level {
 ...
 */
export function createP0P4Calculator(config: Partial<typeof DEFAULT_CONFIG> = {}): P0P4Calculator {

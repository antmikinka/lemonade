...
export class MetricsExtractor {
     ...
     * ```
     */
    extractReach(label, description) {
     ...
     * ```
     */
    extractEffort(label, description) {
        const text = `${label} ${description || ''}`.toLowerCase();
        // Look for time-related patterns
        const timePatterns = [
            { pattern: /(\d+(?:\.\d+)?)\s*(week|weeks)/i, toMonths: (w) => w / 4 },
            { pattern: /(\d+(?:\.\d+)?)\s*(month|months)/i, toMonths: (m) => m },
            { pattern: /(\d+(?:\.\d+)?)\s*(day|days)/i, toMonths: (d) => d / 20 },
            { pattern: /(\d+(?:\.\d+)?)\s*(hour|hours)/i, toMonths: (h) => h / 160 },
            { pattern: /(\d+(?:\.\d+)?)\s*(sprint|sprints)/i, toMonths: (s) => s * 0.5 },
            { pattern: /(\d+(?:\.\d+)?)\s*(quarter|quarters)/i, toMonths: (q) => q * 3 }
        ];
     ...
     * ```
     */
    extractImpact(label, description) {
        const text = `${label} ${description || ''}`.toLowerCase();
     ...
     * ```
     */
    extractConfidence(label, description) {
        const text = `${label} ${description || ''}`.toLowerCase();
     ...
     */
    extractValue(label, description) {
        const text = `${label} ${description || ''}`.toLowerCase();
     ...
     */
    extractUrgency(label, description) {
        const text = `${label} ${description || ''}`.toLowerCase();
     ...
     */
    extractImportance(label, description) {
        const text = `${label} ${description || ''}`.toLowerCase();
     ...
     */
    parseNumberWithUnit(numStr, unit) {
     ...
     */
    calculateExtractionConfidence(label, description) {
        const text = `${label} ${description || ''}`.toLowerCase();
